...
function MultiStepForm({
  ...
  const currentStep = normalizedSteps.find(s => (s.order || s.step_number) === currentStepOrder) || normalizedSteps[0];
  const currentStepIndex = normalizedSteps.findIndex(s => (s.order || s.step_number) === currentStepOrder);
  ...
  useEffect(() => {
    if (normalizedSteps.length > 0) {
      const stepForOrder = normalizedSteps.find(s => (s.order || s.step_number) === currentStepOrder);
      if (!stepForOrder && normalizedSteps.length > 0) {
        // If current step order doesn't exist, find the closest one
        const availableOrders = normalizedSteps.map(s => s.order || s.step_number).sort((a, b) => a - b);
        ...
        return (
          <div className="bg-white rounded-xl shadow-lg overflow-hidden">
            <Form className="p-4 sm:p-6 relative" style={{ overflow: 'visible' }}>
              ...
              <QuestionList questions={sortedStepQuestions} stepId={currentStep.id} readOnly={readOnly} />
              ...
              <FormActions
                ...
                onNext={() => handleNext(validateForm, setTouched, setErrors, values, errors, setSubmitting)}
              />
              )}
            </Form>
          </div>
