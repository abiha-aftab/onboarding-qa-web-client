...
function MultiStepForm({
  ...
  const isLastStep = currentStepIndex === normalizedSteps.length - 1
  ...
  const handleNext = useCallback(
    async (validateForm, setTouched, setErrors, values, existingErrors) => {
      try {
        ...
        const nextIndex = Math.min(currentStepIndex + 1, normalizedSteps.length - 1)
        setCurrentStepIndex(nextIndex)

        if (onStepChange) {
          onStepChange(nextIndex, normalizedSteps[nextIndex])
        }
  ...
  const handleBack = useCallback(() => {
    ...
    if (onStepChange) {
      onStepChange(prevIndex, normalizedSteps[prevIndex])
    }
  }, [currentStepIndex, normalizedSteps, onStepChange])
  ...
  const handleSubmit = useCallback(
    ...
    [currentStepIndex, currentStep, onSubmit, onStepComplete]
  )
            ...
            <div className="mt-8 space-y-4 max-w-2xl mx-auto">
              {normalizedSteps.map(step => {
                ...
                return (
                  <div
                    key={step.id}
                    className="bg-white rounded-lg p-4 shadow-sm border-2 border-[#FFD350]"
                  >
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <h3 className="font-bold text-lg" style={{ color: '#0F5E7B' }}>
                        {step.title}
                      </h3>
                    </div>
  ...
  const sortedStepQuestions = [...(currentStep.step_questions || [])].sort(
    (a, b) => (a.order || 0) - (b.order || 0)
  )
  ...
  return (
    ...
    >
      {({ validateForm, setTouched, setErrors, isSubmitting, values, errors, touched }) => {
        const currentStepHasErrors = sortedStepQuestions.some(stepQuestion => {
          const question = stepQuestion.question
          const key = `question_${question.id}`
        ...
        return (
          <div className="bg-white rounded-xl shadow-lg overflow-hidden">
                      ...
                      {index < normalizedSteps.length - 1 && (
                        <div
                          className={`flex-1 h-1 mx-2 mt-[-20px] transition-all duration-300 ${
                            isCompleted || index < currentStepIndex ? 'bg-[#FFD350]' : 'bg-gray-200'
                          }`}
                        />
                      )}
                    </div>
            ...
            <Form className="p-6 relative">
              {isSubmitting && (
                <div className="absolute inset-0 bg-white bg-opacity-90 rounded-xl flex items-center justify-center z-10">
                  <div className="text-center">
                    ...
                    <p className="text-base font-semibold" style={{ color: '#0F5E7B' }}>
                      Submitting...
                    </p>
                  </div>
                </div>
              ...
              {completedSteps.size > 0 && currentStepIndex > 0 && (
                <div className="mb-6 p-4 bg-gradient-to-r from-cyan-50 to-yellow-50 rounded-lg border-2 border-[#FFD350]">
                  ...
                  <div className="space-y-2">
                    {Array.from(completedSteps).map(completedIndex => {
                      const completedStep = normalizedSteps[completedIndex]
                      return (
              ...
              <div className="space-y-6 mb-6">
                {sortedStepQuestions.map(stepQuestion => {
                  const question = stepQuestion.question
                  return (
                    <DynamicField key={`${currentStep.id}_${question.id}`} question={question} />
                  )
                })}
              </div>
              ...
              <div className="flex justify-between items-center pt-6 border-t border-gray-200">
                ...
                {!isLastStep ? (
                  <button
                    type="button"
                    onClick={() => handleNext(validateForm, setTouched, setErrors, values, errors)}
